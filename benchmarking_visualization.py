...
# Consistent color mapping per grid across all plots
from matplotlib import cm

def grid_color_map(grids, cmap_name="tab20"):
    """Return deterministic color per grid using a fixed colormap.
    The same grid value will map to the same color across functions.
    """
    unique = sorted(set(grids))
    cmap = cm.get_cmap(cmap_name, max(len(unique), 10))
    return {g: cmap(i % cmap.N) for i, g in enumerate(unique)}
...
def plot_best_vs_baseline(data):
    ...
    plt.tight_layout()
    fig.savefig("results/bench_results/best_vs_baseline_combined.png", dpi=300)
...
def strong_scaling(data, mode_name, grid_size, num_itrs):
    ...
    out_path = f"results/bench_results/strong_simple_{mode_name}_g{grid_size}.png"
    fig.savefig(out_path, dpi=300)
    print(f"Saved {out_path}")


def strong_scaling_combined(data, grids, num_itrs):
    """Plot strong scaling for multiple grids with encrypt only."""
    fig, ax = plt.subplots()
    ...
    color_by_grid = grid_color_map(grids)
    ...
    for g in grids:
        ...
        ax.plot(best["p"], best["speedup"], marker=marker, linestyle=linestyle, label=f"{mode_name} g={g}", color=color_by_grid[g])
...
def strong_scaling_runtime(data, grids, num_itrs):
    """Plot strong scaling runtime (minimal runtime_s) for multiple grids (encrypt only)."""
    fig, ax = plt.subplots()
    ...
    if all_p:
        p_sorted = sorted(set(all_p))
        # No ideal line for runtime plot

    for g in grids:
        mode_name = "encrypt"
        df = data[(data["mode"] == mode_name) & (data["grid"] == g)].copy()
        if df.empty:
            print(f"[strong_runtime] no data for mode={mode_name}, grid={g}")
            continue
        ...
        marker = markers[len(marker_by_grid) % len(markers)]
        marker_by_grid[g] = marker
        ax.plot(best["p"], best["runtime_s"], marker=marker, linestyle=linestyle, label=f"{mode_name} g={g}", color=color_by_grid[g])

    ax.set_xlabel("Total cores (np × omp_threads)")
    ax.set_ylabel("Runtime [s]")
    ax.set_title(f"Strong scaling runtime (encrypt only); iters={num_itrs}")
    # ax.set_ylim(0, 200)
    ax.legend()
    ax.grid(True, linestyle=":")
    plt.tight_layout()
    out_path = "results/bench_results/strong_runtime_combined.png"
    fig.savefig(out_path, dpi=300)
    print(f"Saved {out_path}")
...
strong_scaling_runtime(benchmark_data, all_grids, num_itrs)
