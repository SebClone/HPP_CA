...
def strong_scaling_combined(data, grids, num_itrs):
    ...
    ax.grid(True, linestyle=":")
    plt.tight_layout()
    out_path = "bench_results/strong_combined_allgrids.png"
    fig.savefig(out_path, dpi=300)
    print(f"Saved {out_path}")
...
def weak_scaling(data, modes, base_grid, num_itrs):
    ...
    ax.grid(True, linestyle=":")
    plt.tight_layout()
    out_path = f"bench_results/weak_simple_combined_base{base_grid}.png"
    fig.savefig(out_path, dpi=300)
    print(f"Saved {out_path}")
...
benchmark_data = pd.read_csv('bench_results/hpp_bench.csv')
...
# Save to csv
best_overall.to_csv('bench_results/best_configs_overall.csv', index=False)
best_by_mode.to_csv('bench_results/best_configs_by_mode.csv', index=False)
